...
- **Multiline Text Input & Shift+Enter Line Breaking** (`index.html`, `app.js`): Upgraded the primary problem input (`#userInput`) to an accessible, auto-resizing multiline text area. Pressing `Shift+Enter` now creates a new line for composing multi-step equations, proofs, and formatted queries, while pressing `Enter` immediately dispatches the problem. Includes dynamic viewport height auto-expansion up to 180px with scroll containment.

### Fixed & Hardened
- **KaTeX Fraction & Mathematical DOM Order Extraction** (`app.js`): Resolved a display/extraction inversion bug where calling `.textContent` on KaTeX-rendered `.mfrac` DOM nodes extracted denominator elements before numerator elements due to KaTeX's vertical alignment stack (e.g. $\frac{17\pi}{5} \rightarrow \text{"517}\pi\text{"}$, $\frac{7\pi}{5} \rightarrow \text{"57}\pi\text{"}$, $\frac{\pi}{2} \rightarrow \text{"2}\pi\text{"}$). Implemented `latexToMathText()` and `extractKaTeXDomText()` to strictly reconstruct mathematical order ($\text{"17}\pi\text{/5"}$), enabled `htmlAndMathml` output with LaTeX `<annotation>` tags for screen readers, and updated final answer box accessibility labels.
- **Verification Engine Token Capture & Parentheses Arithmetic** (`server/verificationBridge.js`): Corrected a regex capture group flaw in arithmetic extraction that isolated trailing tokens rather than the full expression during multi-token calculations (e.g. $345(180/\pi) = 19768.46$). Restored full arithmetic capture across parentheses, fractional multipliers, and coterminal angle conversions.
- **Form Submission Lock & Concurrency Hardening** (`app.js`): Hardened input submission lifecycle against double-triggering by locking on `isProcessing`, ignoring IME composition (`keyCode 229`) and keyboard repeat events, and explicitly preventing parent `.input-wrapper` form submit bubbling.
